...
User.login = async function(email, password) {
	...
	if (!validator.validate(password).type('string').length(l=>l>0).end()) return Promise.reject(authError);
	if (!validator.validate(email).trim().length(l=>l>0 && l<=320).check(mail => /^[\w.]+@[\w.]+$/.test(mail)).end(({value}) => email = value)) return Promise.reject(authError);
	...
	if (!token || token.expired({deleteonexpiration:true})) {
		try {
			const {token:newToken} = await tokens.create(user.email());
			token = newToken;
		} catch (e) {
			return Promise.reject(e);
		}
