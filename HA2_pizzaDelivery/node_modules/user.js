...
function User(userData) {
	...
	// Setters
	function set(property, v) {

		// exit if no change needs to be made
		if (v === userData[property]) return;
	...
	this.setFirstname = function (v) {
		...
		if (!validator.validate(v).trim().length(l=>l>0).end(({value}) => v = value)) return;
	...
	this.setPassword = function (v) {
		// if (!validator.validate(v).type('string').length(l=>l>0).end() return;

		if (!validator.validate(v).type('string').length(l=>l>0).end()) return;

		const hashed = hash(v, createUserSalt(userData.creationdate, userData.secret));
...
User.login = async function(email, password) {
	...
	if (!validator.validate(password).type('string').length(l=>l>0).end()) {
		return Promise.reject(authError.eat('not valid password'));
	}
	if (!validator.validate(email).trim().length(l=>l>0 && l<=320).check(mail => /^[\w.]+@[\w.]+$/.test(mail)).end(({value}) => email = value)) {
		return Promise.reject(authError.eat('not valid email'));
	}
